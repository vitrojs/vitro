...
import { createComment, createText } from './creators'
...
import { SYMBOL_STORE_OBSERVABLE, context } from '../oby'
import type {
  Child,
  Classes,
  DirectiveData,
...
const setAsChildProps = (parent: Node, child: Node) => {
  if (parent instanceof DocumentFragment && SYMBOL_AS_CHILD_PROPS in parent) {
    if (child instanceof HTMLElement) {
      const asChildProps = parent[SYMBOL_AS_CHILD_PROPS] as Record<string, any>
      if (asChildProps) {
        setProps(child, asChildProps)
      }
    }
  }
}
...
const setProp = (
  ...
  if (isTemplateAccessor(value)) {
    ...
    if (key in element && !isSVG(element)) {
...
const setProps = (
  ...
  if (asChild) {
    ...
    element[SYMBOL_AS_CHILD_PROPS] = object

    setChild(element as unknown as HTMLElement, child)

    return
  }

  for (const key in object) {
    setProp(element as HTMLElement, object, key, isStore(object))
  }
}
